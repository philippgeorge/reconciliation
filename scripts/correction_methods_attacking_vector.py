...
def demand_scaling(g_d, F):
    ...
    for country, df in g_d.items():        
        ...
        f_var["g_d"][country].to_csv("../output/debugging/" + "demand_scaling_" + country + ".csv")
...
def case_approach(g_d, F):
    ...
    for country, df in g_d.items():
        ...
        f_var["g_d"][country]["demand"] = df["gamma"]
        #f_var["g_d"][country].to_csv("../output/debugging/factorial_approach/" + "case_approach_" + country + ".csv")
        #a_var["g_d"][country].to_csv("../output/debugging/factorial_approach/" + "case_approach_a_var_" + country + ".csv")
    a_var["F"] = pd.DataFrame(0, index=np.arange(F.shape[0]), columns=F.columns)
    f_var["F"] = pd.DataFrame(1, index=np.arange(F.shape[0]), columns=F.columns)
    # Print runtime
    end = time()
...
def additive_approach(g_d, F):
    ...
    p_index = {}
    for country, df in g_d.items():  
        p_index[country] = [x for x in df.columns.values if "demand" not in x if "s_r" not in x]
    ...
    model.delta_s_r = pyo.Var(model.n_index, model.t_index, bounds=(0.0, None))
    ...
    def balance_rule(model, n, t):
            return  sum(model.delta_g[n, t, p] + g_d[n][p][t] for p in p_index[n]) + sum(model.delta_F[t, l] + F[l][t] for l in i_index[n]) ==  \
                        model.delta_d[n, t] + g_d[n]["demand"][t] + model.delta_s_r[n, t] + g_d[n]["s_r"][t] + sum(model.delta_F[t, l] + F[l][t] for l in e_index[n])
    ...
    def ObjRule(model):
        return sum(model.delta_g[n, t, p] ** 2 for n in model.n_index for t in model.t_index for p in p_index[n]) \
             ...
             + sum(model.delta_s_r[n, t] ** 2 for n in model.n_index for t in model.t_index)
    ...
    for n in n_index:
        ...
        for t in t_index:
            ...
            row.append(model.delta_s_r[n, t].value)
            table.append(row)
...
def factorial_approach(g_d, F):
    ...
    def balance_rule(model, n, t):
            return  sum(model.alpha[n, t, p] * g_d[n][p][t] for p in p_index[n]) + sum(model.beta[t, l] * F[l][t] for l in i_index[n]) ==  \
                        model.gamma[n, t] * g_d[n]["demand"][t] + model.omega[n, t] * g_d[n]["s_r"][t] + sum(model.beta[t, l] * F[l][t] for l in e_index[n])
    model.balance_rule = pyo.Constraint(model.n_index, model.t_index, rule=balance_rule)
    def ObjRule(model):
        return sum(model.alpha[n, t, p] ** 2 for n in model.n_index for t in model.t_index for p in p_index[n]) \
             ...
             + sum(model.omega[n, t] ** 2 for n in model.n_index for t in model.t_index)
    model.obj = pyo.Objective(rule=ObjRule, sense=pyo.minimize)
    opt = SolverFactory("gurobi", solver_io="python")
    # Solve the model
    opt.solve(model)
...
def internal_sigma_approach(g_d, F, A=100, eta=0.1):
    ...
    def ObjRule(model):
        return sum(model.delta_g[n, t, p] ** 2 * float(sigma[n][p].iloc[t]) for n in model.n_index for t in model.t_index for p in p_index[n]) \
             ...
             + sum(model.delta_s_r[n, t] ** 2 * float(sigma[n]["s_r"].iloc[t]) for n in model.n_index for t in model.t_index)
    model.obj = pyo.Objective(rule=ObjRule, sense=pyo.minimize)
    opt = SolverFactory("gurobi", solver_io="python")
    # Solve the model
    opt.solve(model)
    ...
    for n in n_index:
        ...
        if not os.path.exists("../output/debugging/factorial_approach/"):
            os.makedirs("../output/debugging/factorial_approach/")
        a_var["g_d"][n].to_csv("../output/debugging/factorial_approach/" + "internal_sigma_a_var" + n + ".csv")
    #delta_F    
    table = []
    for t in t_index:
        row = []
...
def external_sigma_approach(g_d, F, g_d_complete, F_complete):
    # Measure the runtime
    start = time()
    # Load sigma, the confidence coefficient for generation, demand and links
    sigma = aux.calculate_external_sigma_av(g_d, F, aux.g_2018_entsoe, aux.d_2018_entsoe, aux.g_by_source, aux.F_default, aux.F_specific, g_d_complete, F_complete)
    ...
    def ObjRule(model):
        return sum((model.alpha[n, t, p] * sigma["g"][n][p]) ** 2 for n in model.n_index for t in model.t_index for p in p_index[n]) \
             ...
             + sum((model.omega[n, t] * sigma["s_r"][n]) ** 2 for n in model.n_index for t in model.t_index)
